class QueueItemsController < ApplicationController
  ...
  def current_user_queued_videos?(video)
    current_user.queue_items.map(&:video).include?(video)
  end

  def queue_video(video)
    QueueItem.create(video: video, user: current_user, position: new_queue_item_position) unless current_user_queued_videos?(video)
  end
